...
const handleLoadProject = useCallback((id: string) => { const data = projectManager.loadProject(id); if (data) { setWalls(data.walls || []); setDimensions(data.dimensions || []); elements.setAllElements(data.columns || [], data.windows || [], data.doors || [], data.northArrows || []); regionsManager.setRegions(data.regions || []); textManager.setAllTexts(data.texts || []); stairManager.setAllStairs(data.stairs || []); setAxes(data.axes || []); savedSnapshotRef.current = JSON.stringify(data); setIsDirty(false); const project = projectManager.projects.find(p => p.id === id); if (project) setCurrentProjectName(project.name); } }, [elements, regionsManager, textManager, stairManager, projectManager]);
